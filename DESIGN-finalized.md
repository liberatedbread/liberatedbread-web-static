...
**When Phase 0 ends:** When the device catalog reaches 5+ guides and the full landing page design (Prompt B, §14.2) is validated, replace `index.md` with the full landing page. `_data/devices.yml` currently has 2, so the count gate is not yet met; the remaining pre-launch items (real device photos, pcfweb integration testing, landing page design review at all breakpoints) are tracked in PHASE0.md. The subscribe form stays in both versions.
...
| v3.2 | 2026-07-28 | Open Questions section resolved — 8 of 9 formerly-open items carry [DECISION], the 9th is [REMOVED]. Added `/faq/`, `/privacy/`, `/security/` and a custom 404; footer and nav updated to match. Device front matter's protocol reference keys renamed `opengreeniot_spec`/`opengreeniot_docs` → `protocol_spec`/`protocol_docs`, with the old names still honoured. |
